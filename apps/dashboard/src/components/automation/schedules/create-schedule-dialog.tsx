...
export function CreateScheduleDialog({
  ...
  useEffect(() => {
    if (open) {
      if (isEditMode) {
        ...
        // Preset quick-start (URL state): prefill output, cadence and
        // lookback, then let the user tweak anything before saving.
        // Auto-publish resets so its switch state never lies.
        form.reset({
          ...getDefaultScheduleValues(),
          outputType: presetValues.outputType,
          schedule: presetValues.schedule,
          lookbackWindow: presetValues.lookbackWindow,
          autoPublish: false,
        });
      } else {
        form.reset(getDefaultScheduleValues(editTrigger));
  ...
  }, [open, editTrigger, form, presetValues, isEditMode]);
